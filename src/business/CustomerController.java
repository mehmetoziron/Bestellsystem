...
public class CustomerController {
    ...
    public boolean update(Customer customer) {
        if (this.getById(customer.getId()) == null) {
            Helper.showMsg(customer.getId() + "Keine registrierten Kunden mit ID gefunden.");
            return false;
        }
    ...
    public boolean delete(int id) {
        if (this.getById(id) == null) {
            Helper.showMsg(id + "Keine registrierten Kunden mit ID gefunden.");
            return false;
        }
...
//            SELECT * FROM customer WHERE name LIKE '%mehmet%' AND type = 'PERSON'
        }
